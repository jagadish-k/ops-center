...
export const tenantsTable = pgTable('tenants', {
   ...
   *         anchor: { x: 250, y: 250 }
   *       }],
   *       poi: [{ id: "p1", type: "MEDICAL", lat: 40.7, lng: -74.0 }]
   *     }]
   *   }
   */
...
export const tenantAccessPeriodsTable = pgTable('tenant_access_periods', {
  id: uuid('id').primaryKey().defaultRandom(),
  tenantId: text('tenant_id')
    .notNull()
    .references(() => tenantsTable.id, { onDelete: 'cascade' }),
  type: text('type').notNull(), // 'EVENT', 'SETUP', 'TEARDOWN'
  startDate: timestamp('start_date', { withTimezone: true }).notNull(),
  endDate: timestamp('end_date', { withTimezone: true }).notNull(),
  notes: text('notes'),
});

export const tenantContactsTable = pgTable('tenant_contacts', {
  id: uuid('id').primaryKey().defaultRandom(),
  tenantId: text('tenant_id')
    .notNull()
    .references(() => tenantsTable.id, { onDelete: 'cascade' }),
  ...
  phone: text('phone'),
  role: text('role'), // 'SPOC', 'BILLING', 'TECHNICAL'
});

export const tenantsRelations = relations(tenantsTable, ({ many }) => ({
  accessPeriods: many(tenantAccessPeriodsTable),
  contacts: many(tenantContactsTable),
}));

export const tenantAccessPeriodsRelations = relations(
  tenantAccessPeriodsTable,
  ({ one }) => ({
    tenant: one(tenantsTable, {
      fields: [tenantAccessPeriodsTable.tenantId],
...
export const tenantContactsRelations = relations(
  ...
  ({ one }) => ({
    tenant: one(tenantsTable, {
      fields: [tenantContactsTable.tenantId],
      references: [tenantsTable.id],
    }),
  }),
);
...
export const usersTable = pgTable(
  ...
  (t) => [
    index('idx_users_global_role').on(t.globalRole),
...
export const userPermissionsTable = pgTable(
  ...
  (t) => [
    primaryKey({ columns: [t.userId, t.permissionName] }),
    index('idx_user_perms_user').on(t.userId),
  ],
);
...
export const staffRosterTable = pgTable(
  ...
  (t) => [
    uniqueIndex('idx_staff_user_tenant').on(t.userId, t.tenantId),
    index('idx_staff_tenant').on(t.tenantId, t.status),
  ],
);
...
export const incidentsTable = pgTable(
  ...
  (t) => [index('idx_inc_tenant_updated').on(t.tenantId, t.updatedAt)],
);
...
export const dispatchesTable = pgTable(
  ...
  (t) => [
    index('idx_dispatch_staff').on(t.targetStaffPhone, t.status),
    index('idx_dispatch_tenant').on(t.tenantId, t.status),
  ],
);
...
export const auditLedgerTable = pgTable(
  'audit_ledger',
  {
  ...
  (t) => [index('idx_audit_tenant_ts').on(t.tenantId, t.timestamp)],
);
