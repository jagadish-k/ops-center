...
interface MutateFn {
  (
    ...
    optimisticUpdate: (draft: AdminPolicy[]) => void,
  ): Promise<boolean>;
}
...
function PolicyEditor({
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col gap-3">
      ...
      {/* Test runner */}
      <details
        className="rounded border border-slate-300 dark:border-slate-800 bg-slate-100 dark:bg-slate-900/40"
        open
      >
        ...
        <div className="flex flex-col gap-2 border-t border-slate-300 dark:border-slate-800 p-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="font-mono text-[10px] text-slate-900 dark:text-slate-500 dark:text-slate-500">
              Evaluates <code>data.stadium.authz.allow</code> with the input
              above
            </span>
          </div>
          ...
          {testResult && (
            ...
            >
              {!testResult.ok ? (
                ...
                <div>
                  <strong>
                    Decision: {testResult.allowed ? 'ALLOW' : 'DENY'}
                  </strong>
...
function CreatePolicyDrawer({
  ...
  const submit = async () => {
    ...
    try {
      ...
      setSubmitError(
        err instanceof ApiError ? err.message : 'Failed to create policy',
      );
    } finally {
      setSubmitting(false);
    }
  ...
  return (
    ...
    >
      <div className="flex h-full flex-col gap-4 p-6">
        <h3 className="font-mono text-sm font-black uppercase tracking-widest text-slate-800 dark:text-slate-100">
          New Policy
        </h3>
