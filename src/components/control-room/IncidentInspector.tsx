  ...
  const mutate = async (
    ...
    try {
      ...
      setFeedback({
        kind: 'ok',
        text: result.queued
          ? `${action.toUpperCase()} queued — will sync.`
          : `${action.toUpperCase()} confirmed by server.`,
      });
    } catch (err) {
      const text =
  ...
  const handleSelectStaff = async (staffPhone: string): Promise<void> => {
    ...
    try {
      ...
      setFeedback({
        kind: 'ok',
        text: result.queued
          ? 'Dispatch queued — will sync.'
          : 'Dispatch sent to staff member.',
      });
      ...
      <div className="min-h-0 flex-1 overflow-y-auto p-5 no-scrollbar">
        ...
        <dl className="grid grid-cols-2 gap-3 font-mono text-[11px]">
          ...
          <Field
            label="Severity"
            value={sev.label}
            valueClass={sev.className}
          />
          <Field
            label="Sector"
            value={incident.extractedMetadata.locationSector}
          />
          <Field
            label="Source"
            value={incident.source.replace('_', ' ')}
          />
          <Field
            label="Grid"
...
function Field({
  ...
  return (
    <div className="rounded-lg border border-slate-300 dark:border-slate-800 bg-slate-50 dark:bg-slate-950/50 px-3 py-2">
      <dt className="text-[9px] uppercase tracking-widest text-slate-600">
        {label}
      </dt>
      <dd
        className={`mt-0.5 font-bold uppercase tracking-wide text-slate-600 dark:text-slate-300 ${valueClass ?? ''}`}
      >
        {value}
      </dd>
    </div>
