...
function tierColor(tier: InfoTier): string {
  switch (tier) {
    case 1:
      return '#ef4444'; // life safety — red
    case 2:
      return '#f97316'; // tactical — orange
    case 3:
      return '#f59e0b'; // crowd/logistics — amber
    case 4:
      return '#3b82f6'; // facilities — blue
    case 5:
      return '#64748b'; // advisory — slate
  }
}
...
export function OptimizedStadiumMapCanvas({
  ...
  const [selectedStaff, setSelectedStaff] = useState<WhitelistUser | null>(
    null,
  );
  ...
  const renderBackground = (dims: Dimensions): void => {
    ...
    const layoutData = propsRef.current.mapLayout as
      MapLayout | null | undefined;
    ...
    if (layoutData?.floors?.length) {
      ...
      const floor =
        layoutData.floors.find((f) => f.id === floorId) ?? layoutData.floors[0];
      if (floor) {
        // Draw zones.
  ...
  useEffect(() => {
    ...
    const render = (): void => {
      ...
      for (const member of staff) {
        ...
        if (activeFloorId && member.floorId && member.floorId !== activeFloorId)
          continue;
      ...
      for (const incident of inc) {
        ...
        if (
          activeFloorId &&
          incident.floorId &&
          incident.floorId !== activeFloorId
        )
          continue;
      ...
      const centerGrid: MapCoordinates = {
        x: Math.round(
          (cssWidth / 2 - vp.offsetX) /
            (baseScale(Math.min(cssWidth, cssHeight)) * vp.zoom),
        ),
        y: Math.round(
          (cssHeight / 2 - vp.offsetY) /
            (baseScale(Math.min(cssWidth, cssHeight)) * vp.zoom),
        ),
      };
      ...
      if (nowMs - fpsLastUpdate.current >= 500) {
        const fps = Math.round(
          (fpsFrames.current * 1000) / (nowMs - fpsLastUpdate.current),
        );
      ...
      const activeLayout = propsRef.current.mapLayout as
        MapLayout | null | undefined;
  ...
  const trackDrag = (e: React.PointerEvent<HTMLCanvasElement>): void => {
    if (dragRef.current.active && pointersRef.current.has(e.pointerId)) {
      const p = toCss(e.clientX, e.clientY);
      if (
        Math.hypot(p.x - dragRef.current.lastX, p.y - dragRef.current.lastY) > 4
      ) {
  ...
  return (
    ...
    >
      {layoutData?.geoBounds && (
        <LeafletMapBackground
          ref={leafletMapRef}
          bounds={layoutData.geoBounds}
          mapProvider={layoutData.mapProvider}
        />
      )}
      <canvas
        ref={canvasRef}
      ...
      {selectedStaff &&
        (() => {
          ...
          const staffIncidents = incidents.filter(
            (inc) =>
              inc.reportedBy === selectedStaff.userId ||
              inc.reportedBy === selectedStaff.id,
          );
          return (
                  ...
                  <span
                    className={
                      selectedStaff.status === 'AVAILABLE'
                        ? 'text-emerald-400'
                        : selectedStaff.status === 'DISPATCHED'
                          ? 'text-amber-400'
                          : 'text-slate-900 dark:text-slate-500'
                    }
                  >
              ...
              {staffIncidents.length === 0 && (
                <div className="mt-3 border-t border-slate-300 dark:border-slate-700 pt-2">
                  <p className="text-[10px] text-slate-600">
                    No reports filed by this person.
                  </p>
                </div>
              )}
            </div>
      ...
      <div className="absolute bottom-3 left-3 z-20 rounded-xl border border-slate-300 bg-white/90 p-3 backdrop-blur neu-raised-sm dark:border-slate-800 dark:bg-slate-950/90">
        ...
        <div className="space-y-1">
          <p className="text-[9px] uppercase text-slate-900 dark:text-slate-500 dark:text-slate-600">
            Specialty
          </p>
          {(
            [
        ...
        <div className="mt-2 space-y-1">
          ...
          {(
            ...
            <Checkbox
              key={status}
              isSelected={filters.incidentStatuses.has(
                status as IncidentStatus,
              )}
              onChange={() => {
                setFilters((prev) => {
                  const next = new Set(prev.incidentStatuses);
                  if (next.has(status as IncidentStatus))
                    next.delete(status as IncidentStatus);
