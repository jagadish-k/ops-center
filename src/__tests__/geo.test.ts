...
import { gpsToGrid, haversineMeters, type BoundingBox } from '../../netlify/lib/geo';
...
describe('gpsToGrid', () => {
	...
	it('projects the NW corner to (0, 0)', () => {
		const result = gpsToGrid(METLIFE_BBOX.maxLat, METLIFE_BBOX.minLng, METLIFE_BBOX);
		expect(result.x).toBe(0);
		expect(result.y).toBe(0);
	});

	it('projects the SE corner to (1000, 1000)', () => {
		const result = gpsToGrid(METLIFE_BBOX.minLat, METLIFE_BBOX.maxLng, METLIFE_BBOX);
