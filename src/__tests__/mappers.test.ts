...
describe('mapIncident', () => {
  ...
  it('maps all fields correctly', () => {
    ...
    expect(result.extractedMetadata.actionRequired).toBe(
      'Deploy relief cordons',
    );
  });
...
describe('mapDispatch', () => {
  ...
  it('maps all fields correctly', () => {
    ...
    expect(result.sentTimestamp).toBe(
      new Date('2026-07-16T12:05:00Z').getTime(),
    );
  ...
  it('maps ack_at and resolved_at when present', () => {
    ...
    expect(result.ackTimestamp).toBe(
      new Date('2026-07-16T12:06:00Z').getTime(),
    );
    expect(result.resolvedTimestamp).toBe(
      new Date('2026-07-16T12:15:00Z').getTime(),
    );
  });
