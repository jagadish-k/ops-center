...
describe('SHA-256 chain mathematics', () => {
	...
	it('produces a deterministic hash for the same payload', () => {
		const payload = buildPayload({
			...
			deltaSHA: sha256(JSON.stringify({ before: { status: 'OPEN' }, after: { status: 'ACKNOWLEDGED' } })),
	...
	it('chain linkage: entry 2 hash depends on entry 1 hash', () => {
		...
		const payload1 = buildPayload({ ...baseEntry, chainedPriorHash: GENESIS_HASH });
		const hash1 = sha256(payload1);

		// Entry 2 chains to hash1.
		const entry2 = { ...baseEntry, eventId: 'evt_002', action: 'ACTION_B', timestamp: 1700000001000 };
		...
		const payload2Tampered = buildPayload({ ...entry2, chainedPriorHash: tamperedHash1 });
	...
	it('different state deltas produce different deltaSHA values', () => {
		const delta1 = sha256(JSON.stringify({ before: { status: 'OPEN' }, after: { status: 'ACKNOWLEDGED' } }));
		const delta2 = sha256(JSON.stringify({ before: { status: 'OPEN' }, after: { status: 'RESOLVED' } }));
